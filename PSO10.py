# Version: 1.0
# Author: ChatGPT & Bevlin Reddy
# Initial version: 24 Oct 2023
...
def objective_function(x):
    return (x[0]**2 -2*x[0] -48)

# Define the combined constraint function
def constraint_function(x):
    # Constraint 1: x + y >= 1
    constraint1 = max(0, x[0] - 1)
    # Constraint 2: x - y <= 2
    constraint2 = max(0, x[0] - 2)
    return constraint1 + constraint2  # Combine constraints with penalty

# Define the bounds for each variable
lower_bound = [-10]  # Lower bounds for x and y
upper_bound = [10]  # Upper bounds for x and y
